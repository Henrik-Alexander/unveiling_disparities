...
births$age_group <- sub("\\[", "", as.character(births$age_group))
births$age_group <- sub("\\)", "", as.character(births$age_group))
births$age_group <- sub("\\]", "", as.character(births$age_group))

# Rates
dta <- births %>% 
  filter(year == 2018 & bundesland == "Hamburg" & age_group != "50,55" & age_group != "55,60")


# Conversion factor
conv_fct <- mean(dta$pop) / mean(dta$asfr)

# TFR ratio
tfr_ratio <- sum(dta$asfr[dta$sex == "male"]) / sum(dta$asfr[dta$sex == "female"])

# Populations
ggplot(dta, aes(x = age_group, colour = sex, fill = sex)) + 
  geom_col(aes(y = pop), position = "dodge", alpha = 0.3) +
