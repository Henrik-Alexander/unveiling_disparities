...
def inset_subplot(box, region, data, variable = "mac", offset = 0.1, n = 10, size = 100, fontsize = 10):
    ...
    for country in unique_countries:
        ...
        ax_inset.scatter(tmp[v_female], tmp[v_male], c = col_dict[country], marker = marker_dict[country],
                edgecolor = "white", linewidths = 0.3, s = size, alpha = 1, label = country)
    ax_inset.plot([min_tfr, max_tfr], [min_tfr, max_tfr], c = "#000000", zorder = 0)
...
min_tfr = math.floor(tfrs.tfr_female.min() * (1 - 0.005))
max_tfr = math.ceil(tfrs.tfr_male.max() * (1 + 0.005))
unique_countries = tfrs.country.unique()

# If you want to alter the order
unique_countries = ["United States", "Germany", "France", "Spain", "Mexico", "Finland", "Colombia", "Australia"]
unique_countries = list(reversed(unique_countries))
