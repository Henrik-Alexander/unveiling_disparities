...
# Assemble the plot
(expert + data_based + stable + impact) +
  plot_layout(guides = "collect", nrow=1, axes = "collect_y", axis_titles = "collect") & 
  theme(legend.text = element_text(size=8),
        legend.key.size = unit(0.2, "cm"),
        title=element_text(size=8))
ggsave(last_plot(), filename = "figures/birth_squeezes_panel_presentation.pdf",
       height = 8, width = 25, unit = "cm")
...
e_country_shares <- plot_country_shares(label="Expert-based approach")
d_country_shares <- plot_country_shares(data_based_approach, "Data-based appraoch")
s_country_shares <- plot_country_shares(stable_pop_approach, "Stable-pop. approach")
...
e_country_shares + d_country_shares + s_country_shares + o_country_shares +
  plot_layout(guides = "collect", nrow = 1, axes = "collect_y", axis_titles = "collect_x") &
  theme(legend.key.size = unit(0.2, "cm"),  title = element_text(size=12))
ggsave(last_plot(), filename = "figures/share_birth_squeeze_presentation.pdf",
       height=8, width=25, unit = "cm")
