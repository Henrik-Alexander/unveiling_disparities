...
### Confersion for Schoen's U -------------------

# Convert the TFR ratio to U
R_to_U <- function(R) {
  (R-1)/(0.5*(R+1))
}

# Convert the Schoen's U to R
U_to_R <- function(U) {
   
}

### Transform TFR for women -----------------------

tfr_to_ratio <- function(tfr_f) {
  lower <- 0.66 + 0.136 * tfr_f
  upper <- 0.85 + 0.136 * tfr_f
