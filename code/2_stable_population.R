...
# Load the Gulcynsky data
srya <- read_xlsx(list.files("U:/data/global/sex_ratios", pattern = "^padr", full.names=T),
                  sheet=3, skip=3)
range(c(srya$Rural, srya$Urban), na.rm=T)
...
leslies <- t_m |> group_by(country, Year) |> mutate(na = sum(!is.na(ASFR))) |> filter(na >0)
...
# Estimate the stable population growth rate
leslies <- t_f |> group_by(country, Year) |> mutate(na = sum(!is.na(ASFR))) |> filter(na >0)
leslies <- split(leslies, list(leslies$country, leslies$Year), drop = T)
...
stable_pop$U_stable_pop <- with(stable_pop, (tfr_ratio_stable_pop-1) / (0.5*(tfr_ratio_stable_pop+1)))
